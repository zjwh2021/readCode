...
level **g_test_vectors;


level **g_test_vectors1;
...
// netlist2graph
char g_strGateTypeFileName[100] = "", g_strGraphFileName[100] = "";
FILE *g_fpGraphFile, *g_fpGateTypeFile;
// netlist2graph
...
char compact = 'n';		/* n: no compaction, r: reverse, s: shuffle */
...
int main(int argc, char *argv[]) //for windows
{
	...
	int bit = 0,packet = 0;
	int bPhase2; //,nd;
	g_test_vectors = new level*[MAXTEST / 10];
	g_test_vectors1 = new level*[MAXTEST / 10];
	g_test_store = new level*[MAXTEST / 10];
	g_test_store1 = new level*[MAXTEST / 10];
	for (int i=0; i<MAXTEST / 10; i++) {
		g_test_vectors[i] = new level[MAXPI + 1];
		g_test_vectors1[i] = new level[MAXPI + 1];
		g_test_store[i] = new level[MAXPI + 1];
		g_test_store1[i] = new level[MAXPI + 1];
	}
	...
	*/
	...
	if (g_strGraphFileName[0] == '\0') {
		while ((c = g_strCctPathFileName[i++]) != '\0')
		{
			if (c == '/')
			{
				...
				g_strGraphFileName[j++] = c;
			}
		}
		g_strGraphFileName[j] = '\0';
		strcat(g_strGraphFileName, "_graph.txt");
	}

	i = 0; j = 0;
	if (g_strGateTypeFileName[0] == '\0') {
		while ((c = g_strCctPathFileName[i++]) != '\0')
		{
			if (c == '/')
			{
				...
				g_strGateTypeFileName[j++] = c;
			}
		}
		g_strGateTypeFileName[j] = '\0';
		strcat(g_strGateTypeFileName, "_gatetype.txt");
	}
	...
	// net2graph start
	if ((g_fpGraphFile = fopen(g_strGraphFileName, "w")) == NULL)
	{
		fprintf(stderr, "Fatal error: %s file open error\n", g_strTestFileName);
		exit(0);
	}

	if ((g_fpGateTypeFile = fopen(g_strGateTypeFileName, "w")) == NULL)
	{
		fprintf(stderr, "Fatal error: %s file open error\n", g_strTestFileName);
		exit(0);
	}
	...
	/*fprintf(g_fpGateTypeFile, "%d\n", g_iNoPI);
	for (int i = 0; i < g_iNoGate; i++) {
		fprintf(g_fpGateTypeFile, "%d,%d\n", g_net[i]->index, g_net[i]->type);
		fprintf(g_fpGraphFile, "%d", g_net[i]->index);
		for (int j = 0; j < g_net[i]->outCount; j++) {
			fprintf(g_fpGraphFile, " %d", g_net[i]->outList[j]->index);
		}
		fprintf(g_fpGraphFile, "\n");
	}
	fclose(g_fpGateTypeFile);
	fclose(g_fpGraphFile);
	return 0;*/

//	fprintf(g_fpGateTypeFile, "%d\n", g_iNoPI);


	// g_iNoGate此时为5， 而 iNoGate = 4;
	for (int i = 0; i < g_iNoGate; i++) {
		fprintf(g_fpGateTypeFile, "%d %d\n", g_net[i]->index, g_net[i]->type);

		// 显示 g_net[0] 即GATE链表的尾巴 结点1；
		// g_net[0]->outCount 为 1； 1 只是 15的输入；

		// g_fpGateTypeFile  即为 类似的GATE 文件， 但是是 index 和 type;

		// 
		for (int j = 0; j < g_net[i]->outCount; j++) {
			fprintf(g_fpGraphFile, "%d %d\n", g_net[i]->index,
				g_net[i]->outList[j]->index);
		}
		// 输出  0  2；
		// 输出边的 index ， 结点1 结点15； 结点1 输出到 结点15； 15 = OR(1, 2);
		// 结点2 结点15： 1 2；
		// 结点2 结点16 ； 1 3；
	...
	if (chCctMode == ISCAS89)
	{
		...
		// dpi 已经遍历完，用2个栈 广度优先遍历；
		// g_iMaxLevel 为 new PO level;
	...
	// g_iNoGate = 5, 增加了 PO结点；; g_iNoPI = 2;
