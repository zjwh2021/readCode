...
extern level **g_test_vectors;
extern level **g_test_vectors1;
extern level **g_test_store;
extern level **g_test_store1;
	...
	*piShuffle = 0;
	...
	while ((!bDone)) //One shuffle
	{
		...
		if (bRunShuffle == TRUE)
		{
			...
			randomizePatterns_FSIM((level (*)[MAXPI+1])g_test_store, (level (*)[MAXPI+1])g_test_vectors, iPacket_Local, iBit_Local, iNoPI);
	...
	*piShuffle = 0;
	...
	while ((!bDone)) //One shuffle
	{
		...
		if (bRunShuffle == TRUE)
		{
			...
			iPacket = iPacket_Local;
			/*shuffles the fpTestFile patterns and stores it back in the random fashion*/
			randomizePatterns_HOPE((level (*)[MAXPI+1])g_test_store, (level (*)[MAXPI+1])g_test_store1, (level (*)[MAXPI+1])g_test_vectors, (level (*)[MAXPI+1])g_test_vectors1, iPacket_Local, iBit_Local, iNoPI);
			//g_test_store -----------> g_test_vectors
			iBit_Local = iPacket_Local = 0;
			for (iCompactCnt = 0; iCompactCnt <= g_iMaxCompact - 1; iCompactCnt++) //iCompactCnt = 0, 1
			{
